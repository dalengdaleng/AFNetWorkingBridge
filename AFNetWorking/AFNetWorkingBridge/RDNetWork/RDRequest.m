...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        ...
        [RDNetworkPrivate addDoNotBackupAttribute:path];
    }
}
...
- (NSString *)cacheBasePath {
    ...
    NSArray *filters = [[RDNetworkConfig sharedInstance] cacheDirPathFilters];
    if (filters.count > 0) {
        for (id<RDCacheDirPathFilterProtocol> f in filters) {
...
- (NSString *)cacheFileName {
    ...
    NSString *baseUrl = [RDNetworkConfig sharedInstance].baseUrl;
    ...
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@ AppVersion:%@ Sensitive:%@",
                             ...
                             argument, [RDNetworkPrivate appVersionString], [self cacheSensitiveData]];
    NSString *cacheFileName = [RDNetworkPrivate md5StringFromString:requestInfo];
